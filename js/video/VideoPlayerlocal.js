...
export default class VideoPlayer extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            isPaused: false,  // 是否暂停，控制视频的播放和暂停
            isLoading:true,
            showIndexAdTime:0,
            duration: 0,     // 视频的时长
    ...
    onPanResponderMove = (evt, gestureState) => {
        ...
        //亮度
        if (Math.abs(gestureState.x0) < (this.state.videoWidth/2) && Math.abs(gestureState.dy) > 20) {
            this.$isShowBrightness = true;
            let value = this.state.brightness-(gestureState.dy/this.state.videoHeight)*.05;
            if (value < 0){
                value = 0;
            }
            if (value >1){
        ...
        //音量
        if (Math.abs(gestureState.x0) > (this.state.videoWidth/2) && Math.abs(gestureState.dy) > 20) {
            let value = this.state.volume-(gestureState.dy/this.state.videoHeight)*.05;
            if (value < 0){
                value = 0;
            }
            if (value >1){
