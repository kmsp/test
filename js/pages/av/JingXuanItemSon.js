...
export default class JingXuanItemSon extends Component {
    ...
    render(){
        ...
        return(
            item.is_ad === 0 ?
                <TouchableOpacity key={item.id+'-'+Math.random(1,100)} activeOpacity={1} onPress={() => {
                    this.props.navigation.goBack();
                    item.type === 1 ?
                        this.props.navigation.navigate('AvDetail',{tabId: item.id})
                        :
                        this.props.navigation.navigate('MoveDetail',{tabId: item.id})
                }}>
                    <View style={styles.itemson}>
                        <FastImage style={{height:((width-30)/2)*0.667,width:(width-30)/2,borderRadius:5,backgroundColor:'#eee'}} source={{uri:((item.img).startsWith('http') ? item.img : global.ActiveDomain + item.img) }} resizeMode={'cover'} resizeMethod={'resize'} />
                        <View style={styles.outermost}>
                            <View style={styles.top}>
                                {item.vip === 2 ?
                                    <Theme.Text style={{height:15,marginRight:5,backgroundColor:'#f4bd3f',color:'@WriteTextColor',paddingHorizontal:5,borderRadius:5,lineHeight:15,paddingVertical: 0,overflow:'hidden',fontSize:10}}>限免</Theme.Text>

                                    :
                                    <Theme.Text style={{height:15,marginRight:5,backgroundColor:'#7D3ED3',color:'@WriteTextColor',paddingHorizontal:5,borderRadius:5,lineHeight:15,paddingVertical: 0,overflow:'hidden',fontSize:10}}>VIP</Theme.Text>
                                }
                            ...
                            <View style={styles.bottom}>
                                ...
                                <View style={styles.bottomright}><Text style={styles.font}>{item.time}</Text>
                                </View>
                            </View>
                        </View>
                ...
                <TouchableOpacity key={item.id.toString()} activeOpacity={1} onPress={() => this._clickAd(item)} >
                    <View style={styles.itemson}>
                        <ImageBackground style={{height:((width-30)/2)*0.667,width:(width-30)/2,margin:0}} source={{uri:global.ActiveDomain+item.img}} resizeMode='cover'>
                        </ImageBackground>
