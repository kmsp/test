...
let pageNo = 0;//当前第几页
let itemNum = 5;    //一页显示数量

export default class AvList extends Component {
    ...
    getData( pageNo ) {
        FetchRequest(global.ActiveDomain+"/Movies",
            "ENCRYPTO",
            {
                account: global.UniqueId,
                page: pageNo,
                num: itemNum,
                nav: this.props.tabId,
                state: "2",
                type: "1"
            },
            (responseJson) => {
        ...
        return (
            <Theme.View style={[styles.container,{backgroundColor:'@BackgroundColor',flex:1}]}>
                <FlatList
                    ...
                    renderItem={(item)=><AvListSon item={item.item} is_like={item.item.like} {...this.props}/>}
