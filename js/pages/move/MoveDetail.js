...
import {
    ...
    FlatList,
    StatusBar,
    Linking, Modal, Image, Clipboard, DeviceEventEmitter
} from 'react-native';
...
import MoveItemSon from "../../common/MoveItemSon";
...
export default class MoveDetail extends Component{
    ...
    getDetail() {
        FetchRequest(
            global.ActiveDomain+"/Movie_details",
            "ENCRYPTO",
            {
            ...
            (responseJson) => {
                if (responseJson.code === 200) {
                    if(responseJson.data.movie_url.length === 0){
                        ...
                        this.setState({
                            AllDate: responseJson.data,          //全部数据
                            MovieDetail: responseJson.data.movie,
                            MovieUrl: responseJson.data.movie_url,         //视频播放连接列表
                            PlayerUrl: responseJson.data.movie_url[0].url,  //默认第一条连接给播放器
                            actor: responseJson.data.actor,
                            label: responseJson.data.label,
                            ad: responseJson.data.ad,
                            ad1: responseJson.data.ad1,
                            isZan: responseJson.data.zan,        //是否点赞
                            zan_lv: responseJson.data.zan_lv + '%',    //点击百分比
                            isLike: responseJson.data.like,          //是否收藏
                            doBug: false,                            //问题反馈
                            doBugTxt: '',                            //问题反馈内容
                            isLoading: false,
                        });
            ...
            (responseJson) => {
              if(responseJson!=""){
                  ...
                  if (pageNo === 0){
                      ...
                      dataBlob = this.state.moreListDate.concat(data);
                  }
    ...
    //问题反馈 提交
    _doPostBugTxt(){
        fetch(global.ActiveDomain+"/feedback", {
            method: "POST",
            headers: {
    ...
    _ListHeaderComponent(){
        ...
        return(
            <Theme.View style={[styles.top,{backgroundColor:'@BackgroundColor'}]}>
                <View style={{flexDirection:'row',paddingHorizontal:10,marginTop:10}}>
                    ...
                    <Theme.Text style={{color:'@BlackTextColor',flex:1,fontWeight: 'bold',padding:0}} numberOfLines={2}>
            ...
            <View style={{flexDirection:'row',paddingHorizontal:10,marginTop:14}}>
                ...
                <View style={{flexDirection:'row',paddingHorizontal:10,marginTop:10}}>
                    <MdTouchableOpacity onPress={()=>this.setState({showNetwork:!this.state.showNetwork})} style={{flex:1,flexDirection:'row',alignItems:'center'}}>
                        <MdIcon name={'bars'} color={'@TextColor'} />
                    ...
                    <MdTouchableOpacity style={{paddingHorizontal:8,alignItems:'center'}}  onPress={()=>this._showTags()}>
                        ...
                        <Theme.Text style={{color:'@LabelTextColor',fontSize:12}}>标签</Theme.Text>
                    </MdTouchableOpacity>
                    <MdTouchableOpacity onPress={()=>this._sharePage()} style={{paddingHorizontal:8,alignItems:'center'}}>
                        ...
                        <Theme.Text style={{color:'@LabelTextColor',fontSize:12}}>分享</Theme.Text>
                    </MdTouchableOpacity>
                    <MdTouchableOpacity onPress={()=>this._doDownload(this.state.PlayerUrl)} style={{paddingHorizontal:8,alignItems:'center'}}>
                        ...
                        <Theme.Text style={{color:'@LabelTextColor',fontSize:12}}>下载</Theme.Text>
                    </MdTouchableOpacity>
                     {
                        this.state.isLike === 1 ?
                            <Theme.Text onPress={()=>this._collection(0)} style={{color:'@WriteTextColor',borderWidth:1,borderColor:'@ThemeColor',backgroundColor:'@ThemeColor',padding:5,borderRadius:5,overflow:'hidden',marginLeft:10}}>已收藏</Theme.Text>
                            :
                            <Theme.Text onPress={()=>this._collection(1)} style={{color:'@LabelTextColor',borderWidth:1,borderColor:'@LabelTextColor',padding:5,borderRadius:5,marginLeft:10}}>收藏影片</Theme.Text>
                    }
                </View>
                ...
                <View style={{height:5,backgroundColor:'#eee',marginTop:10}} />
                <Theme.Text style={{marginTop:10,marginLeft:10,fontSize:14,color:'@TextColor'}}>相关影片</Theme.Text>
            </Theme.View>
        )
    }
    ...
    render(){
        ...
        // const url = this.state.MovieDetail.url;
        // const url = 'http://1256993030.vod2.myqcloud.com/d520582dvodtransgzp1256993030/7732bd367447398157015849771/v.f30.mp4';
