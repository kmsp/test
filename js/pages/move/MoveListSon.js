...
export default class MoveListSon extends Component {
    ...
    render(){
        ...
        return(
            item.is_ad === 0 ?
                <TouchableOpacity style={styles.itemson} key={item.id.toString()} activeOpacity={1} onPress={() => this.props.navigation.push('MoveDetail',{tabId: item.id,playTime:Math.floor(item.playtime)})} {...this.props}>
                    <FastImage style={{height:((width-30)/2)*0.667,width:(width-30)/2,backgroundColor:'#eee',borderRadius:5}} source={{uri:((item.img).startsWith('http') ? item.img : global.ActiveDomain + item.img)}}resizeMode={'cover'} />
                ...
                :
                <TouchableOpacity style={styles.itemson} key={item.id.toString()} activeOpacity={1} onPress={() => this._clickAd(item)} >
                    <FastImage style={{height:((width-30)/2)*0.667,width:(width-30)/2,margin:0,borderRadius:5}} source={{uri:((item.img).startsWith('http') ? item.img : global.ActiveDomain + item.img)}} resizeMode='cover' />
